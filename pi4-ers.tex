...
\begin{center}
	\begin{tabular}{@{} l p{10.5cm} @{}}
		...
		Usuário Comum          & Usuário que pode comprar e vender produtos                      \\
...
\begin{itemize}
	...
	\item informações públicas do vendedor.
\end{itemize}
...
O sistema deve permitir que o comprador adicione um produto ao carrinho.
\begin{itemize}
...
O sistema deve permitir que o comprador remova um produto do carrinho.

\subsection{RF14 -- Alterar quantidade do produto no carrinho}
O sistema deve permitir que o comprador altere a quantidade de um produto no carrinho.
\begin{itemize}
...
O sistema deve permitir que o comprador consulte o carrinho.
...
O sistema deve permitir que o comprador crie um pedido com os produtos do carrinho.
\begin{itemize}
...
O sistema deve permitir que o comprador consulte seus pedidos.
\begin{itemize}
	\item \textbf{RN29} -- O comprador somente pode consultar pedidos associados à sua conta.
\end{itemize}

\subsection{RF18 -- Consultar pedidos recebidos}
O sistema deve permitir que o vendedor consulte pedidos que possuem produtos anunciados por ele.
\begin{itemize}
	\item \textbf{RN30} -- O vendedor somente pode consultar pedidos relacionados aos seus anúncios.
\end{itemize}
...
O sistema deve permitir que o comprador avalie uma compra.
...
\begin{itemize}
	...
	\item \textbf{RN34} -- Somente o comprador responsável pelo pedido pode criar a avaliação;
	\item \textbf{RN35} -- Cada compra pode possuir somente uma avaliação do comprador;
	\item \textbf{RN36} -- A nota deve possuir um valor entre 1 e 5.
\end{itemize}

\subsection{RF21 -- Consultar avaliações}
O sistema deve permitir que os usuários consultem as avaliações públicas de um vendedor.
\begin{itemize}
	\item \textbf{RN37} -- Somente avaliações registradas devem fazer parte da nota pública do vendedor.
\end{itemize}
...
O sistema deve responder a 95\% das operações de consulta em até 2 segundos no ambiente definido para os testes.
\begin{itemize}
	\item \textbf{RN43} -- O tempo deve ser medido entre a solicitação do usuário e o recebimento da resposta do sistema.
\end{itemize}

\subsection{RNF02 -- Segurança da comunicação}
O sistema deve proteger os dados durante a comunicação entre o usuário e a plataforma.
\begin{itemize}
	\item \textbf{RN44} -- Dados sensíveis não devem ser transmitidos em formato desprotegido.
\end{itemize}

\subsection{RNF03 -- Proteção de credenciais}
O sistema deve proteger as credenciais dos usuários.
\begin{itemize}
	\item \textbf{RN45} -- A senha do usuário não deve ser exibida em texto legível;
	\item \textbf{RN46} -- A senha do usuário não deve ser armazenada em formato diretamente legível.
\end{itemize}

\subsection{RNF04 -- Controle de acesso}
O sistema deve impedir o acesso a funções que não pertencem à permissão do usuário.
\begin{itemize}
	\item \textbf{RN47} -- Somente administradores podem acessar funções administrativas;
	\item \textbf{RN48} -- Um usuário não pode alterar dados privados de outro usuário.
\end{itemize}

\subsection{RNF05 -- Responsividade}
O sistema deve permitir o uso das funções principais em diferentes tamanhos de tela.
\begin{itemize}
	\item \textbf{RN49} -- As funções principais devem estar disponíveis em computadores, tablets e celulares.
\end{itemize}

\subsection{RNF06 -- Compatibilidade}
O sistema deve funcionar nos navegadores definidos para o projeto.
\begin{itemize}
	\item \textbf{RN50} -- A equipe deve definir os navegadores suportados antes dos testes de aceitação.
\end{itemize}

\subsection{RNF07 -- Clareza das mensagens}
O sistema deve informar quando uma operação não puder ser concluída.
\begin{itemize}
	\item \textbf{RN51} -- A mensagem deve informar o motivo do erro quando essa informação estiver disponível;
	\item \textbf{RN52} -- A mensagem não deve apresentar informações técnicas internas ao usuário comum.
\end{itemize}
...
\begin{itemize}
	\item \textbf{RN53} -- O sistema não deve concluir uma operação quando um campo obrigatório estiver vazio;
	\item \textbf{RN54} -- O sistema não deve aceitar valores que violem as regras definidas para o requisito.
\end{itemize}
...
\begin{itemize}
	...
	\item chat entre comprador e vendedor;
...
Os requisitos funcionais e não funcionais devem ser rastreados até as regras de
negócio relacionadas. A tabela a seguir apresenta a rastreabilidade completa:
...
\begin{center}
	\begin{tabular}{@{} l l @{}}
		...
		RNF01 -- Desempenho                         & RN43                                    \\
		RNF02 -- Segurança da comunicação           & RN44                                    \\
		RNF03 -- Proteção de credenciais            & RN45, RN46                              \\
		RNF04 -- Controle de acesso                 & RN47, RN48                              \\
		RNF05 -- Responsividade                     & RN49                                    \\
		RNF06 -- Compatibilidade                    & RN50                                    \\
		RNF07 -- Clareza das mensagens              & RN51, RN52                              \\
		RNF08 -- Integridade dos dados              & RN53, RN54                              \\
		\bottomrule
	\end{tabular}
\end{center}
\end{document}
